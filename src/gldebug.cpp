...
void glDebug::createDebugGeometry(){
	...
	grid_2_line_group = glDebug::generateDebugGrid("grid10.0", 0.5f, 200, 1.0f, 1.0f, 1.0f, 0.3f);
        ...
        float pColorR, float pColorG, float pColorB, float pAlpha)
{
	...
	unsigned int grid_color = 2;//gl::createConstColor(glm::vec4(pColorR, pColorG, pColorB, pAlpha), pName + "_color").color_index;
...
unsigned int glDebug::initCoordinateSystem(std::string pName)
{
	...
	unsigned int red_color_i = 3;//gl::getConstColor("red");
	unsigned int green_color_i = 4;//gl::getConstColor("green");
	unsigned int blue_color_i = 5;//gl::getConstColor("blue");
