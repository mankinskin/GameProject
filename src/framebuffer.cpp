...
void texture::initGUIFBO() {
	...
	quadIndexTexture = get2DTextureID(createTexture2D(gl::Viewport::current->width, gl::Viewport::current->height, GL_R16UI, GL_RED_INTEGER, GL_UNSIGNED_INT, 0));
	//fontColorTexture = get2DTextureID(createTexture2D(gl::Viewport::current->width, gl::Viewport::current->height, GL_RGBA8, GL_RGBA, GL_UNSIGNED_BYTE, 0));
	...
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, gl::Viewport::current->width, gl::Viewport::current->height);
	...
	quadIndexBuffer = vao::createStorage(gl::Viewport::current->width * gl::Viewport::current->height * sizeof(unsigned int), nullptr, GL_MAP_READ_BIT | vao::MAP_PERSISTENT_FLAGS);
...
void texture::initGBuffer()
{
	...
	gAmbientTexture = get2DTextureID(createTexture2D(gl::Viewport::current->width, gl::Viewport::current->height, GL_RGBA16F, GL_RGBA, GL_FLOAT, 0));
	gDiffuseTexture = get2DTextureID(createTexture2D(gl::Viewport::current->width, gl::Viewport::current->height, GL_RGBA16F, GL_RGBA, GL_FLOAT, 0));
	gSpecularTexture = get2DTextureID(createTexture2D(gl::Viewport::current->width, gl::Viewport::current->height, GL_RGBA16F, GL_RGBA, GL_FLOAT, 0));
	gPosTexture = get2DTextureID(createTexture2D(gl::Viewport::current->width, gl::Viewport::current->height, GL_RGB16F, GL_RGB, GL_FLOAT, 0));
	gNormalTexture = get2DTextureID(createTexture2D(gl::Viewport::current->width, gl::Viewport::current->height, GL_RGB16F, GL_RGB, GL_FLOAT, 0));
